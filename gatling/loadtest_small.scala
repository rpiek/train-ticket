...
class LoadTestSmall extends Simulation {
  ...
  val circularFeederIds = Iterator.continually(Map("id" -> circularIds.next()))
  ...
  setUp(
    scn3.inject(
      rampUsersPerSec(1) to 12 during (100 seconds),
      constantUsersPerSec(12) during (30 seconds)
    ).protocols(httpConf1),
    scn4.inject(
      rampUsersPerSec(1) to 6 during (100 seconds),
      constantUsersPerSec(6) during (30 seconds)
    ).protocols(httpConf2) 
  .andThen(
    scn1.inject(constantUsersPerSec(12) during (5 minutes)).protocols(httpConf1),
    scn2.inject(constantUsersPerSec(6) during (5 minutes)).protocols(httpConf2)
  )
  )
}
